...
class MTVUutisetCollector(NewsCollector):
    """Collector for MTV Uutiset articles"""

    
    def __init__(self):
        super().__init__('mtv_uutiset')
        # Use centralized RSS feeds configuration
        self.rss_feeds = {feed.category: feed.url for feed in APIConfig.RSS_FEEDS['mtv_uutiset']}
    ...
    def get_articles_from_rss(self, category: str = 'politiikka', limit: int = 50) -> List[MTVArticle]:
    ...
    def search_articles(self, query: str, limit: int = 20) -> List[MTVArticle]:
        """Search for articles using MTV Uutiset search"""
        try:
            search_url = f"{self.get_base_url()}/haku"
            params = {'q': query}
            
            self.logger.info(f"Searching MTV Uutiset for: {query}")
    ...
    def get_politician_articles(self, politician_name: str, limit: int = 20) -> List[MTVArticle]:
        """Get articles mentioning a specific politician"""
        return self.search_articles(politician_name, limit)

    def _parse_rss_entry(self, entry, category: str) -> Optional[MTVArticle]:
    ...
    def _parse_search_result(self, element) -> Optional[MTVArticle]:
...
if __name__ == "__main__":
    ...
    # Save data
    collector.save_data(articles, "mtv_articles.json")
    def collect_data(self, **kwargs) -> List[Dict[str, Any]]:
        """Implement abstract method from base class"""
