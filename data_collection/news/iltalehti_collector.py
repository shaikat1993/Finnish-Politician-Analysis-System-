...
class IltalehtCollector(NewsCollector):
    """Collector for Iltalehti (IL.fi) articles"""

    
    def __init__(self):
        super().__init__('iltalehti')
    ...
    def get_articles_from_rss(self, category: str = 'politiikka', limit: int = 50) -> List[IltalehtArticle]:
    ...
    def search_articles(self, query: str, limit: int = 20) -> List[IltalehtArticle]:
        """Search for articles using IL.fi search"""
        try:
            search_url = f"{self.get_base_url()}/haku"
            params = {'q': query}
            
            self.logger.info(f"Searching Iltalehti for: {query}")
    ...
    def _parse_search_result(self, element) -> Optional[IltalehtArticle]:
...
if __name__ == "__main__":
    ...
    # Save data
    collector.save_data(articles, "iltalehti_articles.json")
    def collect_data(self, **kwargs) -> List[Dict[str, Any]]:
        """Implement abstract method from base class"""
